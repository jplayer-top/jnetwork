    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        ...
        if (elements != null && elements.size() > 0) {
            for (Element eee : elements) {
                AutoWired elementAnnotation = eee.getAnnotation(AutoWired.class);
                PackageElement packageOf = processingEnv.getElementUtils().getPackageOf(eee);

                TypeElement classElement = (TypeElement) eee.getEnclosingElement();
